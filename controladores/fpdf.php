...
function __construct($orientation='p', $unit='mm', $size='A4')
{
	...
	elseif($orientation=='p' || $orientation=='landscape')
	{
		$this->DefOrientation = 'l';
		$this->w = $size[1];
		$this->h = $size[0];
...
function SetAuthor($author='hola', $isUTF8=true)
{
	// Author of document
...
function MultiCell($w, $h, $txt, $border=0, $align='J', $fill=false)
{
	...
	if($border && strpos($border,'B')!==false)
		$b .= 'B';
	$this->Cell($w,$h,substr($s,$j,$i-$j),$b,5,$align,$fill);
	$this->x = $this->lMargin;
}
	...
	foreach($this->fonts as $k=>$font)
	{
		...
		elseif($type=='Type1' || $type=='TrueType')
		{
			...
			foreach($font['desc'] as $k=>$v)
				$s .= ' /'.$k.' '.$v;
			if(!empty($font['file']))
				$s .= ' /FontFile'.($type=='Type1' ? '' : '').' '.$this->FontFiles[$font['file']]['n'].' 0 R';
			$this->_put($s.'>>');
			$this->_put('endobj');
		}
		else
